...
4. 系統匣選單頂部會顯示同一份用量摘要，作為 tooltip 在特定桌面環境不穩定時的 fallback。
5. 在系統匣圖示上執行預設動作可回到正常面板顯示方式。
6. 系統匣右鍵選單提供 `Show` / `Exit` 作為跨平台 fallback。
...
手動確認雙按面板會隱藏視窗並出現系統匣圖示，hover tooltip 或系統匣選單顯示用量資訊，雙按圖示或點選 `Show` 可恢復面板。
